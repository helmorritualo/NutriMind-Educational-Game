...
- For LiteraQuest Mission 1 gameplay HUD/modals, build Canvas at 1920×1080 with QuizAssets sliced sprites and LiberationSans TMP (same typography as Quiz Portal); use **NutriMind → LiteraQuest Mission 1 → Setup Mission Canvas UI** for one-shot scene setup; **Wire G5 Area01 Mission Controller** and **Wire Term1 Mission Select Controller** for scene wiring.
...
- LiteraQuest Term 1: mission scene `Assets/_Project/Nutrimind/Scenes/App/Literaquest Term/LiteraQuest_Term1_Mission1.unity`; nav Worldhub → `LiteraQuestTerms` → `Term_1_SelectMissions` → `LoadingTransition` open → mission via registry keys `LiteraQuestTerm1Missions`/`LiteraQuestTerm1Mission1`, `MissionSelectionController` + `SubjectTermStore.PendingSceneKey`; canvas setup via `LiteraQuestMission1CanvasSetupEditor.cs`.
- `PlayerMovementController` on `Player`; disable `PlayerModel` `CapsuleCollider`; snaps to `PlayerSpawnPoint`, grounds feet via raycast; Joystick Pack `Fixed Joystick` + `JumpButton `; Input System + legacy keyboard fallback; tuned defaults walk **3.5** / run **7** / acceleration **14**.
- `SimpleCameraFollow` on `Main Camera` → `Player`; third-person offset `(0, 3, -6.5)`, `lookAtOffset` `(0, 1.4, 0)`.
- `G5Area01MissionController` on `Systems/G5Area01MissionController `; mission scripts under `Runtime/Gameplay/Missions/`; wire via **NutriMind → LiteraQuest Mission 1 → Wire G5 Area01 Mission Controller**; `GateBlocker` is a `BoxCollider` child under `GateToNextArea ` (controller toggles it, not `ClosedGate` MeshCollider); Area 1 flow: Farmer Lira interact → dialogue → 3 questions → `StoryMapFragment` → gate open → `AreaCompletePanel`.
- Mission 1 object names often have trailing spaces (`G5Area01MissionController`, `FarmerLira_NPC`, `GateToNextArea`, `ClosedGate`, `StoryMapFragment`, `JumpButton`); align `NPC_InteractionMarker` to `NPC_Model` local position with `SphereCollider` radius ~2 for close-range Talk prompt.
